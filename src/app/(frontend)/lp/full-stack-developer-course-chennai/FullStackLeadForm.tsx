...
export function FullStackLeadActions({ compact = false, directions: showDirections = false, sticky = false }: { compact?: boolean; directions?: boolean; sticky?: boolean }) {
  if (sticky) return <><a href={`tel:${phone}`} onClick={() => track('full_stack_phone_click')}><Phone size={17} />Call</a><a href={whatsapp} target="_blank" rel="noreferrer" onClick={() => track('full_stack_whatsapp_click')}><MessageCircle size={17} />WhatsApp</a><button type="button" onClick={focusForm}>Enquire</button></>
  return <div className={`${styles.actions} ${compact ? styles.actionsCompact : ''}`}>
    ...
    <a href={`tel:${phone}`} onClick={() => track('full_stack_phone_click')}><Phone size={17} />{compact ? 'Call' : 'Call Now'}</a>
