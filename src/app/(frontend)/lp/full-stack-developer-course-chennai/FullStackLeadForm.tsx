...
const whatsapp = `https://wa.me/918939069135?text=${encodeURIComponent('Hi SRT, I would like details about the Full Stack Developer Course in Chennai.')}`
...
function focusForm() {
  const form = document.getElementById('full-stack-lp-enquiry')
  form?.scrollIntoView({ behavior: 'smooth', block: 'center' })
  window.setTimeout(() => form?.querySelector<HTMLInputElement>('input[name="name"]')?.focus({ preventScroll: true }), 550)
}

export function FullStackLeadActions({ compact = false, directions: showDirections = false, sticky = false }: { compact?: boolean; directions?: boolean; sticky?: boolean }) {
  if (sticky) return <><a href={`tel:${phone}`} onClick={() => track('full_stack_phone_click')}><Phone size={17} />Call {displayPhone}</a><a href={whatsapp} target="_blank" rel="noreferrer" onClick={() => track('full_stack_whatsapp_click')}><MessageCircle size={17} />WhatsApp</a><button type="button" onClick={focusForm}>Enquire</button></>
  return <div className={`${styles.actions} ${compact ? styles.actionsCompact : ''}`}>
    ...
    {showDirections && <a href={directions} target="_blank" rel="noreferrer" onClick={() => track('full_stack_directions_click')}><MapPin size={17} />Get Directions</a>}
    <a href={`tel:${phone}`} onClick={() => track('full_stack_phone_click')}><Phone size={17} />Call {displayPhone}</a>
    <a href={whatsapp} target="_blank" rel="noreferrer" onClick={() => track('full_stack_whatsapp_click')}><MessageCircle size={17} />{compact ? 'WhatsApp' : 'WhatsApp Us'}</a>
  </div>
}
...
export function FullStackLeadForm() {
  ...
  if (updateToken) return <form className={styles.form} id="full-stack-lp-enquiry" onSubmit={submitDetails}>
    ...
    <button className={styles.skip} type="button" onClick={() => window.location.assign('/thank-you/full-stack-enquiry/')}>No thanks, continue</button>
